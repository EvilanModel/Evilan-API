...
The TTS API provides high-quality text-to-speech synthesis powered by ElevenLabs, bringing Evilan's voice to life.
...
```json
{
  "text": "Hello! I'm Evilan, your AI assistant."
}
```
...
```bash
curl -X POST "https://evilan.onrender.com/api/tts" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "Welcome to Evilan AI! How can I assist you today?"
  }' \
  --output speech.mp3
```
...
Evilan uses ElevenLabs' `eleven_multilingual_v2` model with optimized settings:
...
| Voice ID | Custom | Evilan's unique voice profile |
...
async function speakText(text) {
  try {
    const response = await fetch('https://evilan.onrender.com/api/tts', {
      method: 'POST',
...
async function chatWithVoice(message) {
  // Get text response
  const chatResponse = await fetch('https://evilan.onrender.com/api/chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      message,
  ...
  // Speak the response
  const ttsResponse = await fetch('https://evilan.onrender.com/api/tts', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
